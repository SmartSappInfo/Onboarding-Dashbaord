            ...
            <DialogContent className="sm:max-w-3xl h-[90vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl text-left bg-background">
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full text-left">
                    <DialogHeader className="p-8 bg-muted/30 border-b shrink-0 text-left">
                        <div className="flex items-center gap-4 text-left">
                            ...
                            <div className="text-left">
                                <DialogTitle className="text-2xl font-black uppercase tracking-tight text-left">Task Studio</DialogTitle>
                                <DialogDescription className="text-xs font-bold uppercase tracking-widest text-muted-foreground text-left">Define objectives contexts for {activeWorkspaceId}.</DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                            ...
                            <div className="p-8 space-y-10 text-left">
                                <div className="space-y-6 text-left">
                                    <div className="space-y-2 text-left">
                                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 text-left">Task Title</Label>
                                        <Input {...register('title')} placeholder="e.g. Conduct operational audit" className="h-14 rounded-2xl bg-muted/20 border-none font-black text-2xl px-6 shadow-inner text-left" />
                                    </div>
                                    <div className="space-y-2 text-left">
                                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 text-left">Execution Brief</Label>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                                    <div className="space-y-4 text-left">
                                        <Label className="text-[10px] font-black uppercase tracking-widest text-primary ml-1 text-left">1. Priority Protocol</Label>
                                        <Controller name="priority" control={control} render={({ field }) => (
                                            <div className="grid grid-cols-2 gap-2 bg-muted/30 p-1.5 rounded-2xl border shadow-inner text-left">
                                                {(['low', 'medium', 'high', 'urgent'] as const).map(p => (
                                                    <button key={p} type="button" onClick={() => field.onChange(p)} className={cn("h-10 rounded-xl font-black uppercase text-[9px] tracking-widest transition-all text-left px-2", field.value === p ? (p === 'urgent' ? "bg-rose-600 text-white shadow-lg" : p === 'high' ? "bg-orange-500 text-white shadow-lg" : "bg-white shadow-lg text-primary") : "text-muted-foreground opacity-60 hover:opacity-100")}>{p}</button>
                                                ))}
                                            </div>
                                        )} />
                                    </div>
                                    <div className="space-y-4 text-left">
                                        <Label className="text-[10px] font-black uppercase tracking-widest text-primary ml-1 text-left">2. Workflow Status</Label>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                                    <div className="space-y-2 text-left">
                                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 flex items-center gap-2 text-left"><User className="h-3 w-3" /> Owner</Label>
                                    ...
                                    <div className="space-y-2 text-left">
                                        <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1 flex items-center gap-2 text-left"><Building className="h-3 w-3" /> {singular} Binding</Label>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                                    <div className="space-y-2 text-left"><Label className="text-[10px] font-black uppercase text-muted-foreground ml-1 flex items-center gap-2 text-left"><Calendar className="h-3 w-3" /> Implementation Start</Label><Controller name="startDate" control={control} render={({ field }) => (<DateTimePicker value={field.value} onChange={field.onChange} />)} /></div>
                                    <div className="space-y-2 text-left"><Label className="text-[10px] font-black uppercase text-primary ml-1 flex items-center gap-2 text-left"><Target className="h-3 w-3" /> Dead-line</Label><Controller name="dueDate" control={control} render={({ field }) => (<DateTimePicker value={field.value} onChange={field.onChange} />)} /></div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 text-left">
                                    <div className="space-y-8 text-left">
                                        <div className="flex items-center justify-between px-1 text-left"><div className="flex items-center gap-2 text-left"><StickyNote className="h-4 w-4 text-primary" /><h4 className="text-xs font-black uppercase tracking-widest text-left">Protocol Ledger</h4></div><Badge variant="secondary">{notes.length}</Badge></div>
                                        <div className="space-y-4 text-left">
                    ...
                    <DialogFooter className="bg-muted/30 p-8 border-t shrink-0 flex justify-between text-left">
                        ...
                        <Button type="submit" disabled={isSaving} className="rounded-2xl font-black h-14 px-16 shadow-2xl bg-primary text-white uppercase tracking-widest text-sm gap-2 active:scale-95 transition-all text-left">{isSaving ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />}Commit Blueprint</Button>
                    </DialogFooter>
                </form>
            </DialogContent>
        </Dialog>
