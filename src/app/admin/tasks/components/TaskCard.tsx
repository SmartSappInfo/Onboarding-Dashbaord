...
export default function TaskCard({ task, isOverlay, onClick }: TaskCardProps) {
 ...
 <h4 className={cn("font-semibold text-xs tracking-tight leading-tight truncate text-foreground", isDone && "line-through opacity-40")}>
                            {task.title}
                        </h4>
                    </div>
 <Avatar className="h-6 w-6 border border-white shadow-sm shrink-0">
                        <AvatarImage src={`https://i.pravatar.cc/150?u=${task.assignedTo}`} />
 <AvatarFallback className="text-[8px] font-semibold">{getInitials(task.assignedToName)}</AvatarFallback>
                    </Avatar>
                </div>
