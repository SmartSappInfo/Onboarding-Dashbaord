...
import { Calendar as CalendarIcon, ArrowLeft, Loader2, Building, MapPin } from 'lucide-react';
  ...
  const onSubmit = async (data: FormData) => {
    if (!firestore || !user) {
      toast({
        ...
        description: "Firestore connection is unavailable or you are not logged in.",
      });
      return;
    }
    ...
    <div className="h-full overflow-y-auto p-4 sm:p-6 md:p-8 bg-muted/5">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col gap-1">
            <h1 className="text-4xl font-black tracking-tight text-foreground">Add New School</h1>
            <p className="text-muted-foreground font-medium">Initialize a new school onboarding profile and assign geographic context.</p>
        </div>
          ...
          <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-8 pb-24">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="border-none shadow-sm ring-1 ring-border rounded-2xl overflow-hidden">
                  <CardHeader className="bg-muted/30 border-b pb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-lg font-black uppercase tracking-tight">Campus Identity</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6 space-y-6">
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="md:col-span-2">
                            ...
                            <FormControl>
                              <Input placeholder="e.g., Ghana International School" {...field} className="h-12 rounded-xl bg-muted/20 border-none shadow-none focus:ring-1 focus:ring-primary/20 font-bold text-lg" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="GIS" {...field} className="h-12 rounded-xl bg-muted/20 border-none shadow-none focus:ring-1 focus:ring-primary/20 font-black text-center" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 ml-1">Motto / Slogan</FormLabel>
                          <FormControl>
                            <Input placeholder="e.g., Knowledge and Character" {...field} className="h-11 rounded-xl bg-muted/20 border-none shadow-none focus:ring-1 focus:ring-primary/20 italic font-medium" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription className="text-[10px] uppercase font-bold tracking-tighter mt-2 ml-1">Select the core SmartSapp features this school requires.</FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
              ...
              <div className="space-y-8">
                ...
                <Card className="border-none shadow-sm ring-1 ring-border rounded-2xl overflow-hidden">
                  <CardHeader className="bg-muted/30 border-b pb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-lg font-black uppercase tracking-tight">Location & Operations</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6 space-y-6">
                    <FormField
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea placeholder="Detailed campus location..." {...field} className="min-h-[80px] rounded-xl bg-muted/20 border-none shadow-none focus-visible:ring-1 focus-visible:ring-primary/20 text-sm p-4" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-col">
                          <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 ml-1">Target Go-Live</FormLabel>
                          <Popover>
                            <PopoverTrigger asChild>
                              <FormControl>
                                <Button
                                  variant={"outline"}
                                  className={cn(
                                    "h-11 justify-start pl-3 text-left font-bold rounded-xl bg-muted/20 border-none shadow-none",
                ...
                <Card className="border-none shadow-sm ring-1 ring-border rounded-2xl overflow-hidden">
                  <CardHeader className="bg-muted/30 border-b pb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <CardTitle className="text-lg font-black uppercase tracking-tight">Branding Assets</CardTitle>
                    </div>
                  </CardHeader>
                  <CardContent className="p-6 space-y-6">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 ml-1">School Logo</FormLabel>
                          <FormControl>
                            <MediaSelect {...field} className="rounded-xl overflow-hidden" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 ml-1">Campus Hero Image</FormLabel>
                          <FormControl>
                            <MediaSelect {...field} className="rounded-xl overflow-hidden" />
                          </FormControl>
                          <FormMessage />
                ...
                {/* Final Submission */}
                <div className="pt-4">
                    <Button type="submit" className="w-full h-14 rounded-2xl font-black text-lg shadow-xl gap-3 transition-all active:scale-95" disabled={methods.formState.isSubmitting}>
                        {methods.formState.isSubmitting ? <Loader2 className="h-6 w-6 animate-spin" /> : <Building className="h-6 w-6" />}
                        Initialize School Profile
                    </Button>
                </div>
              </div>
            </div>
