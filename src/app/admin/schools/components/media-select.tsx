      ...
      <div className={cn("space-y-2 w-full", className)}>
        <div className="flex items-start gap-3">
            ...
            {value && filterType === 'image' && isLikelyImage(value) ? (
                <Image src={value} alt="Preview" fill className="object-contain rounded-[inherit] p-2" />
            ) : (
                <PreviewIcon />
            )}
            </div>
            <div className="flex-grow space-y-3 min-w-0">
                ...
                <div className="flex items-center gap-2">
                    <Button type="button" variant="outline" size="sm" onClick={() => setIsUploadOpen(true)} className="rounded-xl font-black border-primary/10 text-[10px] uppercase tracking-widest h-9 px-4 hover:bg-primary/5 hover:text-primary transition-all">
                        <Upload className="mr-2 h-3.5 w-3.5" /> Upload New
                    </Button>
                    <Button type="button" variant="outline" size="sm" onClick={() => setIsLibraryOpen(true)} className="rounded-xl font-black border-primary/10 text-[10px] uppercase tracking-widest h-9 px-4 hover:bg-primary/5 hover:text-primary transition-all">
          ...
          <SheetHeader className="p-8 border-b bg-muted/30 shrink-0">
            <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                    <SheetTitle className="text-2xl font-black uppercase tracking-tight">Direct Upload</SheetTitle>
                    <SheetDescription className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Upload and optimize institutional branding assets.</SheetDescription>
                </div>
            </div>
          </SheetHeader>
