      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent className="max-w-4xl w-[70vw] p-0 flex flex-col text-left">
            ...
            <SheetTitle className="flex items-center gap-3 text-left">
              {entity.logoUrl ? (
                <div className="relative h-10 w-10 rounded-xl overflow-hidden bg-muted/20 border shadow-sm shrink-0">
                  ...
                  <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-black uppercase shadow-sm shrink-0">
                      {entity.initials || entity.displayName.substring(0, 2)}
                  </div>
              )}
              <div className="text-left">
                <p className="font-black text-xl text-left">{entity.displayName}</p>
                {entity.initials && <Badge variant="secondary" className="text-[10px] font-black uppercase tracking-widest">{entity.initials}</Badge>}
              </div>
            </SheetTitle>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-6 text-left">
                        ...
                        <DetailItem icon={Workflow} label="Lifecycle Status">
                            {entity.lifecycleStatus ? (
                                <Badge variant="outline" className="text-[10px] font-black uppercase tracking-widest bg-primary/5 text-primary border-primary/20 mt-1">
                                    {entity.lifecycleStatus}
                                </Badge>
                            ) : (
                            ...
                            <div className="flex flex-wrap gap-2 pt-1">
                                {entity.modules && entity.modules.length > 0 ? (
                                    entity.modules.map((module) => (
                                        <Badge key={module.id} style={{ backgroundColor: module.color, color: 'white' }} className="border-transparent text-[8px] font-black uppercase">
                                            {module.name}
                                        </Badge>
                                    ))
                                ) : (
          ...
          <SheetFooter className="p-6 mt-auto border-t justify-between flex-row gap-2 text-left bg-muted/10">
            ...
            <Button onClick={() => router.push(`/admin/entities/${entity.entityId}/edit`)} className="rounded-xl font-black h-11 px-6 shadow-lg shadow-primary/20 text-left">
