...
export function ImageEditor({ imageUrl, originalFileName, originalFileSize, imageDimensions, initialState, onStateChange }: ImageEditorProps) {
  ...
  // Handle manual resizing via handles
  const handleResize = (corner: 'tl' | 'tr' | 'bl' | 'br', deltaX: number, deltaY: number) => {
    if (!containerRef.current || !croppedAreaPercent) return;

    const { width: containerWidth, height: containerHeight } = containerRef.current.getBoundingClientRect();
    
    // Convert deltas to percentages relative to container
    const dxPercent = (deltaX / containerWidth) * 100;
    const dyPercent = (deltaY / containerHeight) * 100;

    // Zoom is the only way to "resize" the box relative to the image in react-easy-crop
    // since the box itself is always relative to the container.
    // Effectively, dragging a handle "out" is like zooming "in".
        ...
        <div className="space-y-6">
            <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                    <Input id="filename" value={filename} onChange={(e) => setFilename(e.target.value)} className="h-11 rounded-xl bg-muted/20 border-none shadow-none focus-visible:ring-1 focus-visible:ring-primary/20" />
            ...
            <div className="space-y-2 text-xs text-muted-foreground rounded-2xl bg-primary/5 border border-primary/10 p-4 shadow-inner">
                <p className="font-black uppercase tracking-widest text-primary mb-2">Original File Meta</p>
