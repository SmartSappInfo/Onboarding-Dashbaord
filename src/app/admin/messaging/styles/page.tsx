            ...
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="flex items-center justify-end flex-wrap gap-4">
                    <div className="flex items-center gap-2">
                        <RainbowButton onClick={() => setIsAiGenerating(true)} className="h-11 px-6 gap-2 font-black uppercase text-[10px] tracking-widest shadow-xl">
                ...
                {isAdding && (
                    <Card className="mb-8 border-primary/20 bg-primary/5 animate-in slide-in-from-top-4 duration-300 rounded-[2.5rem] overflow-hidden shadow-xl">
                        <CardHeader className="bg-primary/5 border-b p-8">
                            <CardTitle className="text-xl font-black uppercase tracking-tight">Manual Style Studio</CardTitle>
                            <CardDescription className="text-[10px] font-bold uppercase tracking-widest text-primary/60">Configure high-compatibility email wrappers.</CardDescription>
                        </CardHeader>
                        <CardContent className="p-8 space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Style Label</Label>
                                ...
                                <div className="space-y-4">
                                    <Label className="text-[10px] font-black uppercase text-primary ml-1 flex items-center gap-2"><Share2 className="h-3 w-3" /> Shared Context</Label>
                                    <MultiSelect options={workspaceOptions} value={workspaceIds} onChange={setWorkspaceIds} />
                                </div>
                            </div>
                            <div className="space-y-2">
                                <div className="flex justify-between items-center px-1">
                                    <Label className="text-[10px] font-black uppercase text-muted-foreground">HTML Gateway Blueprint</Label>
                                    <Badge className="bg-orange-500/10 text-orange-600 border-none text-[8px] font-black uppercase h-5 px-2">Must include &#123;&#123;content&#125;&#125;</Badge>
                                </div>
                                <Textarea 
                                    value={htmlWrapper} 
                            ...
                            <div className="flex justify-end">
                                <Button onClick={handleAdd} disabled={isSubmitting || !name || !htmlWrapper.includes('{{content}}')} className="rounded-xl h-12 px-12 font-black uppercase text-[10px] tracking-widest shadow-xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 pb-32">
                    {isLoading ? (
                                ...
                                <div className="aspect-video w-full bg-slate-100 relative overflow-hidden border-b">
                                    ...
                                    {style.workspaceIds?.length > 1 && (
                                        <div className="absolute top-3 left-3">
                                            <Badge className="bg-primary/80 backdrop-blur-md border-none text-[8px] font-black uppercase h-5"><Share2 className="h-2.5 w-2.5 mr-1" /> Shared</Badge>
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="col-span-full py-32 text-center border-4 border-dashed rounded-[4rem] bg-muted/5 flex flex-col items-center justify-center gap-4 opacity-30">
                            ...
                            <p className="font-black uppercase tracking-widest text-xs">No styles in this workspace hub</p>
                        </div>
                    )}
                </div>
            </div>
                ...
                <DialogContent className="max-w-4xl h-[85vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl rounded-[2.5rem]">
                    <DialogHeader className="p-8 bg-muted/30 border-b shrink-0">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-left">
                                <DialogTitle className="text-xl font-black uppercase tracking-tight">Modify Style Architecture</DialogTitle>
                                <DialogDescription className="text-xs font-bold uppercase tracking-widest opacity-60">Updating shared layout protocol</DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    <div className="flex-1 flex flex-col overflow-hidden bg-background">
                        <div className="p-8 border-b grid grid-cols-1 md:grid-cols-2 gap-8 shrink-0">
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Internal Reference</Label>
                                <Input value={editName} onChange={e => setEditName(e.target.value)} className="h-11 rounded-xl bg-muted/20 border-none font-bold" />
                            </div>
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-primary ml-1 flex items-center gap-2"><Share2 className="h-3 w-3" /> Target Workspaces</Label>
                        ...
                        <Tabs defaultValue="code" className="flex-1 flex flex-col min-h-0">
                            <div className="px-8 py-2 bg-muted/10 border-b shrink-0 flex items-center justify-between">
                                <TabsList className="bg-background border p-1 h-9 rounded-xl shadow-sm">
                                    <TabsTrigger value="code" className="text-[9px] font-black uppercase tracking-widest px-4 gap-2"><Code size={14} /> Source</TabsTrigger>
                                    <TabsTrigger value="preview" className="text-[9px] font-black uppercase tracking-widest px-4 gap-2"><Eye size={14} /> Render</TabsTrigger>
                                </TabsList>
                                <Badge className="bg-orange-500/10 text-orange-600 border-none text-[8px] font-black uppercase h-5">Fidelity Assurance Active</Badge>
                            </div>
                    ...
                    <DialogFooter className="p-6 bg-muted/30 border-t shrink-0 flex justify-between items-center sm:justify-between">
                        ...
                        <Button onClick={handleUpdate} disabled={isUpdating || !editName.trim() || editWorkspaceIds.length === 0} className="rounded-xl font-black px-12 shadow-2xl h-12 uppercase tracking-widest text-sm transition-all active:scale-95">
                ...
                <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl rounded-[2.5rem]">
                    <DialogHeader className="p-8 bg-muted/30 border-b shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                ...
                                <DialogDescription className="text-xs font-bold uppercase tracking-widest opacity-60">Generate responsive brand wrappers via AI</DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
                        <div className="w-full lg:w-1/2 p-8 border-r flex flex-col gap-8 overflow-y-auto bg-slate-50">
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Identity Label</Label>
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Design Directives</Label>
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-1">Visual Inspiration (Optional)</Label>
                                <MediaSelect value={aiInspirationUrl} onValueChange={setAiInspirationUrl} filterType="image" className="rounded-2xl" />
                            </div>
                            <RainbowButton onClick={handleAiGenerate} disabled={isAiProcessing} className="h-14 w-full font-black text-lg gap-2 uppercase tracking-widest shadow-2xl">
                        ...
                        <div className="w-full lg:w-1/2 p-8 flex flex-col bg-white">
                            <div className="flex items-center justify-between mb-6">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2"><Eye className="h-3 w-3" /> Live Render</Label>
                                {generatedHtml && <Badge className="bg-emerald-50 text-emerald-600 border-none font-black text-[8px] uppercase tracking-widest">Logic Verified</Badge>}
                            </div>
                            <div className="flex-1 rounded-[2.5rem] bg-slate-50 border-2 border-dashed border-border flex items-center justify-center relative overflow-hidden shadow-inner p-4">
                                {generatedHtml ? (
                                    ...
                                    <div className="text-center space-y-4 opacity-20">
                                        ...
                                        <p className="text-[10px] font-black uppercase tracking-[0.3em]">Awaiting Simulation</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <DialogFooter className="p-6 bg-muted/30 border-t shrink-0 flex justify-between items-center sm:justify-between">
                        ...
                        <Button onClick={handleSaveGenerated} disabled={!generatedHtml || isSubmitting} className="rounded-xl font-black px-12 shadow-2xl h-12 uppercase text-xs tracking-widest active:scale-95 transition-all">
                ...
                <DialogContent className="max-w-3xl h-[80vh] flex flex-col p-0 overflow-hidden border-none shadow-2xl rounded-[2.5rem]">
                    <DialogHeader className="p-8 bg-muted/30 border-b shrink-0">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-xl font-black uppercase tracking-tight">Atmospheric Preview</DialogTitle>
                    ...
                    <DialogFooter className="p-4 bg-white border-t shrink-0">
                        <Button onClick={() => setPreviewStyle(null)} className="w-full h-14 rounded-2xl font-black uppercase tracking-widest text-xs">Exit Preview</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
