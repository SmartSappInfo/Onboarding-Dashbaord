...
function CopyButton({ value, toastTitle, className, icon: Icon = Copy }: CopyButtonProps) {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "h-6 w-6 rounded-md text-muted-foreground hover:text-primary hover:bg-primary/5 transition-all shrink-0 active:scale-[0.97]",
        className
      )}
      onClick={handleCopy}
    >
...
export default function QRStudioClient() {
  ...
  const handlePause = async (qr: QRCodeType) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to pause QR code.' });
    }
  };
  ...
  const handleResume = async (qr: QRCodeType) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to resume QR code.' });
    }
  };

  const handleBulkAction = async (action: 'pause' | 'resume' | 'archive' | 'delete' | 'expire') => {
  ...
  const handleDuplicate = async (qr: QRCodeType) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to duplicate.' });
    }
  };
  ...
  const filteredCodes = React.useMemo(() => {
    ...
    return searched.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'scans':
          cmp = (a.stats?.totalScans || 0) - (b.stats?.totalScans || 0);
          break;
        case 'date':
        default:
  ...
  }, [qrCodes, debouncedSearch, statusFilter, sortField, sortDirection]);
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    if (sortField !== field)
      return <ArrowUpDown className="h-3 w-3 ml-1 opacity-0 group-hover/sort:opacity-50 transition-opacity" />;
  ...
  const statCards = [
    ...
    {
      label: 'Scan Rate',
      value: stats.totalCodes > 0 ? `${Math.round(stats.totalScans / stats.totalCodes)}` : '0',
      icon: BarChart3,
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="rounded-xl h-11 px-6 font-semibold text-sm shadow-lg shadow-primary/20 hover:shadow-primary/30 active:scale-[0.97] transition-all"
            >
          ...
          <AnimatePresence>
            {selectedIds.length > 0 && (
              <motion.div
                ...
                className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 p-3 bg-card border border-primary/20 rounded-2xl shadow-2xl flex items-center gap-4 min-w-[360px]"
              >
        ...
        <Card className="rounded-2xl border-none ring-1 ring-border shadow-sm bg-card overflow-hidden">
          {loading ? (
            ...
            >
              ...
              <div>
                <h3 className="text-lg font-semibold text-foreground">No QR codes found</h3>
                <p className="text-sm text-muted-foreground mt-1 max-w-sm">
                  Create your first QR code or adjust your active filters to start tracking scans.
                </p>
              </div>
              <Button
                onClick={() => router.push('/admin/qr-studio/new')}
                className="mt-2 rounded-xl h-11 px-6 font-semibold shadow-lg shadow-primary/20 active:scale-[0.97]"
              >
                <Plus className="h-4 w-4 mr-2" />
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-border hover:bg-transparent">
                    ...
                    <TableHead className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground w-40 hidden md:table-cell px-2">
                      Short Link
                    </TableHead>
                    <TableHead className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground w-52 hidden sm:table-cell px-2">
                      Destination
                    </TableHead>
                    <TableHead
                ...
                <TableBody>
                  <AnimatePresence>
                    {filteredCodes.map((qr) => {
                      ...
                      const hasExpiry = Boolean(qr.lifecycleConfig?.expiresAt);
                      const hasSchedule = Boolean(qr.lifecycleConfig?.startAt);
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                  ...
                                  <div className="group/name flex items-center gap-1.5 min-w-0">
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                                        <p className="font-semibold text-sm truncate cursor-help flex-1 min-w-0">
                                          {qr.name}
                                        </p>
                                      </TooltipTrigger>
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  {hasSchedule && qr.status === 'scheduled' && (
                                    <span className="inline-flex items-center text-[9px] text-blue-600 dark:text-blue-400 font-medium">
                                      <Calendar className="h-2.5 w-2.5 mr-0.5" /> Scheduled
                                    </span>
                                  )}
                                  {hasExpiry && qr.status !== 'expired' && (
                                    <span className="inline-flex items-center text-[9px] text-amber-600 dark:text-amber-400 font-medium">
                                      <Clock className="h-2.5 w-2.5 mr-0.5" /> Expires
                                    </span>
                                  )}
                                </div>
                              </div>
                          ...
                          <TableCell className="hidden md:table-cell w-40 max-w-[160px] truncate px-2 py-3">
                            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                              <Tooltip>
                                ...
                                <TooltipContent>Status: {statusStyle.label}</TooltipContent>
                              </Tooltip>
                          ...
                          <TableCell onClick={(e) => e.stopPropagation()} className="w-px whitespace-nowrap px-2 py-3">
                            <div className="flex items-center justify-end gap-1">
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/5 transition-all duration-200 active:scale-[0.97]"
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                <DropdownMenuContent align="end" className="rounded-xl w-48">
                                  ...
                                  {qr.status === 'active' || qr.status === 'scheduled' ? (
                                    <DropdownMenuItem
                                      ...
                                      className="rounded-lg cursor-pointer"
                                    >
                                      <Pause className="h-4 w-4 mr-2" /> Pause Link
                                    </DropdownMenuItem>
                                  ) : qr.status === 'paused' || qr.status === 'expired' ? (
                                    <DropdownMenuItem
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        handleResume(qr);
                                      ...
                                      className="rounded-lg cursor-pointer"
                                    >
                                      <Play className="h-4 w-4 mr-2" /> Resume Link
                                    </DropdownMenuItem>
                                  ) : null}
                                  <DropdownMenuSeparator />
                                  {qr.status === 'archived' ? (
            ...
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-sm text-muted-foreground">
                ...
                {archiveTarget?.mode === 'dynamic' && (
                  <> Scans will return a &quot;no longer active&quot; status. This action can be reversed at any time.</>
                )}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
          ...
          <AlertDialogContent className="rounded-2xl">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-sm text-muted-foreground">
                ...
                This action cannot be undone and all scan telemetry will be lost.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel className="rounded-xl">Cancel</AlertDialogCancel>
        ...
        {shareQR && (
          <ShareEmbedDialog
            ...
            publicUrl={
              shareQR.mode === 'dynamic' && shareQR.shortPath
                ? `${window.location.origin}/q/${shareQR.shortPath}`
                : shareQR.destination.url || ''
            }
            embedUrl={
              shareQR.destination.url
                ? shareQR.destination.url.includes('?')
                  ? `${shareQR.destination.url}&embed=true`
                  : `${shareQR.destination.url}?embed=true`
                : ''
            }
          />
