...
export function EmailHygieneHoverCard({ email, hygiene, onManualRecheck, isRechecking, children }: Props) {
  ...
  return (
    <Popover>
      ...
      <PopoverContent side="right" align="start" className="w-80 p-0 bg-slate-900 border-slate-800 shadow-2xl shadow-black overflow-hidden z-50">
        ...
        <div className="bg-slate-950 p-4 border-b border-slate-800 flex items-start gap-4">
          <div className="w-12 h-12 shrink-0 relative flex items-center justify-center">
            ...
            <span className="text-sm font-black text-white z-10">{status === 'unchecked' ? '?' : score}</span>
          </div>
        ...
        <div className="p-4 bg-slate-900/50">
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-800/50">
            ...
            >
              {isRechecking ? <Loader2 size={12} className="animate-spin mr-1.5" /> : <RotateCw size={12} className="mr-1.5" />}
