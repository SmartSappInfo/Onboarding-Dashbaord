...
import type { PortalThemeConfig, PortalBranding, PortalThemeColors } from '@/lib/types/portal';
import {
...
export function PortalThemeCustomizer({
  ...
  const handleColorChange = (key: keyof PortalThemeColors, value: string) => {
    if (paletteMode === 'light') {
      ...
      const currentDark = theme.darkColors || {};
      onChangeTheme({
        ...theme,
        darkColors: {
          ...currentDark,
  ...
  const currentColors = React.useMemo(
    () => resolveActivePortalColors(theme, paletteMode),
    [theme, paletteMode]
  );
      ...
      <Card className="rounded-2xl border-2 border-border shadow-sm">
        ...
        <CardContent className="space-y-6 pt-0">
          ...
          <div className="space-y-1.5 pb-4 border-b border-border">
            <div className="flex items-center justify-between">
              <Label className="text-xs font-bold">Theme Presentation Policy</Label>
              <Badge variant="outline" className="text-[10px] font-mono">
                colorMode
              </Badge>
            </div>
            <Select
              value={theme.colorMode || 'user_choice'}
              onValueChange={(val: 'user_choice' | 'light' | 'dark' | 'system') =>
                onChangeTheme({ ...theme, colorMode: val })
              }
            >
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="user_choice">
                  User Choice — Members can freely switch between Light & Dark modes
                </SelectItem>
                <SelectItem value="light">Strict Light — Always force crisp Light theme</SelectItem>
                <SelectItem value="dark">Strict Dark — Always force immersive Dark theme</SelectItem>
                <SelectItem value="system">Follow Device — Match member&apos;s operating system setting</SelectItem>
              </SelectContent>
            </Select>
          </div>

          {/* ── Dual Palette Switcher (Light vs Dark) ──────────────────── */}
          <div className="flex flex-wrap items-center justify-between gap-3 bg-muted/60 p-1.5 rounded-2xl border border-border">
            <div className="flex items-center gap-1.5">
              <Button
                ...
                className={cn(
                  'h-9 px-4 rounded-xl text-xs font-bold gap-2 transition-all active:scale-[0.97]',
                  paletteMode === 'light'
                    ? 'bg-background shadow-xs text-primary'
                    : 'text-muted-foreground hover:text-foreground'
                )}
              >
                <Sun className="w-3.5 h-3.5 text-amber-500" /> Light Palette
              </Button>
              <Button
                type="button"
                size="sm"
              ...
              >
                <Moon className="w-3.5 h-3.5 text-indigo-400" /> Dark Palette
                {theme.darkColors && (
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 ml-0.5" title="Customized" />
                )}
              </Button>
            </div>
            ...
            {paletteMode === 'dark' && (
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="h-8 px-2.5 rounded-xl text-xs font-bold gap-1.5 text-primary border-primary/30 hover:bg-primary/5 active:scale-[0.97]"
                  title="Auto-generate harmonious dark tokens from your light palette"
                >
                  <Wand2 className="w-3 h-3" /> Auto-Generate Dark
                </Button>
                {theme.darkColors && (
                  <Button
                    type="button"
                    ...
                    className="h-8 px-2 rounded-xl text-xs text-muted-foreground hover:text-rose-500 active:scale-[0.97]"
                    title="Reset to default dark tokens"
                  >
          ...
          <div>
            <Label className="text-xs font-bold mb-2 block">
              Curated Palettes ({paletteMode === 'light' ? 'Light Mode' : 'Dark Mode'})
            </Label>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
              {PRESET_PALETTES.map(p => (
                <button
                  ...
                  className="flex flex-col items-center gap-1.5 p-2 rounded-xl border border-border hover:border-primary/50 hover:bg-muted/50 transition-all text-center active:scale-[0.97]"
                >
          ...
          {/* Individual Pickers — 8 Tokens Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {/* 1. Primary */}
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                <span>Primary</span>
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.primary}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  ...
                  className="w-10 h-10 rounded-xl cursor-pointer border border-border p-0.5 bg-background shrink-0"
                />
                <Input
                  value={currentColors.primary}
                  onChange={e => handleColorChange('primary', e.target.value)}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                <span>Accent</span>
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.accent}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                <span>Secondary</span>
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.secondary}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.background}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                <span>Surface</span>
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.surface}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                <span>Text</span>
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.text}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                ...
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.mutedText}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={currentColors.mutedText}
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-bold flex items-center justify-between">
                <span>Border</span>
                <span className="text-[10px] font-mono text-muted-foreground">{currentColors.border}</span>
              </Label>
              <div className="flex items-center gap-2">
                <input
                  type="color"
