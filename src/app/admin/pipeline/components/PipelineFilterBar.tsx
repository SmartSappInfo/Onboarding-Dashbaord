  ...
  return (
    <div className="shrink-0 mb-5 rounded-2xl border-none ring-1 ring-border shadow-sm bg-card transition-all duration-200">
      <div className="flex flex-wrap items-center gap-2.5 p-3">
        ...
        {/* Status */}
        <Select value={filters.status} onValueChange={(v: 'all' | 'open' | 'won' | 'lost') => updateFilter('status', v)}>
          <SelectTrigger className="h-10 sm:h-9 w-full sm:w-[120px] rounded-xl border border-border bg-background font-bold text-[11px] sm:text-[10px] shadow-sm hover:bg-muted/10 transition-colors">
        ...
        <Select value={filters.assignedToId ?? 'all'} onValueChange={v => updateFilter('assignedToId', v)}>
          <SelectTrigger className="h-10 sm:h-9 w-full sm:w-[140px] rounded-xl border border-border bg-background font-bold text-[11px] sm:text-[10px] shadow-sm hover:bg-muted/10 transition-colors">
            <Users className="h-3.5 w-3.5 sm:h-3 sm:w-3 mr-1 text-muted-foreground/60" />
        ...
        {/* Tags */}
        <MultiSelectPopover
          icon={<TagIcon className="h-3.5 w-3.5 sm:h-3 sm:w-3" />}
          label="Tags"
        ...
        {active && (
          <Button variant="ghost" size="sm" onClick={onClear} className="h-10 sm:h-9 rounded-xl font-bold text-[11px] sm:text-[10px] text-rose-600 hover:bg-rose-50 gap-1.5 px-3 border border-rose-200/50 bg-rose-500/[0.02]">
      ...
      {expanded && (
        <div className="flex flex-wrap items-end gap-4 sm:gap-5 px-3 pb-3 pt-1.5 border-t border-dashed border-border/50">
