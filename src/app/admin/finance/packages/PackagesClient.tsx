...
export default function PackagesClient() {
    ...
    const packagesQuery = useMemoFirebase(() => 
        firestore ? query(collection(firestore, 'subscription_packages'), orderBy('name', 'asc')) : null, 
    [firestore]);
    ...
    const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (editingPackage) {
                ...
                await addDoc(collection(firestore, 'subscription_packages'), packageData);
            ...
            <div className="max-w-6xl mx-auto space-y-8">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground font-medium mt-1">Manage institutional subscription levels and student-based rates.</p>
                    </div>
                    ...
                    <Table>
                        <TableHeader className="bg-muted/30">
                            <TableRow>
                                ...
                                <TableHead className="text-[10px] font-black uppercase tracking-widest">Billing Term</TableHead>
                                    ...
                                    <TableRow key={pkg.id} className="group hover:bg-muted/30 transition-colors">
                                        <TableCell className="pl-8 py-4">
                                            <div>
                                                ...
                                                <p className="text-[10px] text-muted-foreground truncate max-w-[250px]">{pkg.description}</p>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <Badge variant="outline" className="text-[9px] font-black uppercase h-5 border-primary/20 bg-primary/5 text-primary">
                                                {pkg.billingTerm}
                                            </Badge>
                                        </TableCell>
                                        ...
                                        <TableCell className="text-right pr-8">
                                            <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg" onClick={() => setEditingPackage(pkg)}>
                                ...
                                <TableRow>
                                    <TableCell colSpan={5} className="h-64 text-center">
                                        <div className="flex flex-col items-center justify-center gap-3 opacity-20">
                                            ...
                                            <p className="text-xs font-black uppercase tracking-widest">No pricing tiers defined</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            )}
                ...
                <div className="p-6 rounded-[2rem] bg-blue-50 border border-blue-100 flex items-start gap-5 shadow-sm">
                    <div className="p-3 bg-blue-100 rounded-2xl text-blue-600 shadow-sm"><Info className="h-6 w-6" /></div>
                    <div className="space-y-1">
                        <p className="text-sm font-black uppercase tracking-tight text-blue-900">Subscription Integrity</p>
                        <p className="text-[10px] text-blue-700 leading-relaxed font-bold uppercase tracking-widest opacity-80">
                            Updating a package rate will only affect NEW school signups or renewals. Existing invoices are snapshotted at the time of creation to ensure financial consistency.
                        </p>
                    </div>
                </div>
            </div>
                ...
                <DialogContent className="sm:max-w-md rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl">
                    <form onSubmit={handleSave}>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <DialogTitle className="text-2xl font-black uppercase tracking-tight">
                                        {editingPackage ? 'Update Pricing' : 'New Pricing Tier'}
                                    </DialogTitle>
                                    <DialogDescription className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Define student-based subscription logic</DialogDescription>
                                </div>
                            </div>
                        </DialogHeader>
                        <div className="p-8 space-y-8">
                            ...
                            <div className="space-y-2">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Package description</Label>
                                <Textarea name="description" defaultValue={editingPackage?.description} placeholder="Value proposition for the school..." className="min-h-[100px] rounded-xl bg-muted/20 border-none p-4 font-medium" />
                            </div>
                            ...
                            <div className="flex items-center justify-between p-4 rounded-2xl border bg-muted/10">
                                <div className="space-y-0.5">
                                    <Label className="text-sm font-bold uppercase tracking-tight">Active for Signups</Label>
                        ...
                        <DialogFooter className="bg-muted/30 p-6 border-t flex justify-between gap-3 sm:justify-between">
                            <Button type="button" variant="ghost" onClick={() => { setIsAdding(false); setEditingPackage(null); }} className="font-bold rounded-xl px-8">Cancel</Button>
                            <Button type="submit" disabled={isSaving} className="rounded-xl font-black px-12 shadow-2xl h-12 uppercase tracking-widest text-sm">
