        ...
        <div className="flex-1 overflow-y-auto p-6">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-5">
            ...
            <TabsContent value="overview" className="space-y-5 mt-0">
              ...
              {/* Provenance Trail */}
              {prospect.provenance && prospect.provenance.length > 0 && (
                <div className="space-y-2 pt-2 border-t border-border/40">
                  <span className="text-[11px] font-semibold text-muted-foreground flex items-center gap-1">
                    <Info className="w-3 h-3" /> Data Provenance & Trust
                  </span>
                  <div className="space-y-1">
                    {prospect.provenance.map((p, idx) => (
                      <div key={idx} className="flex items-center justify-between text-[11px] text-muted-foreground bg-muted/20 px-2.5 py-1 rounded-lg">
                        <span className="font-medium text-foreground">{p.field}: via {p.source}</span>
                        <Badge variant="secondary" className="text-[10px] h-4 px-1">{p.confidence}% confidence</Badge>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <TabsContent value="tech" className="space-y-4 mt-0">
              <div className="space-y-3">
                <h4 className="text-xs font-semibold text-foreground flex items-center gap-1.5">
                  <Layers className="w-3.5 h-3.5 text-primary" /> Detected Technologies
                </h4>
                {prospect.websiteScan?.technologies && prospect.websiteScan.technologies.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {prospect.websiteScan.technologies.map((t, idx) => (
                      <Badge key={idx} variant="secondary" className="text-xs py-1 px-2.5 font-medium bg-muted/60">
                        {t}
                      </Badge>
                    ))}
                  </div>
                  ...
                  <div className="p-4 rounded-xl border border-dashed border-border text-center text-xs text-muted-foreground">
                    No technographics detected yet. Click <strong>AI Enrich</strong> to run BuiltWith deep scan.
                  </div>
                )}
              </div>
