...
export default function BackofficeStyleEditorPage() {
    ...
    // Data Subscriptions - message_templates globally across all orgs to check style usage
    ...
    React.useEffect(() => {
        ...
        fetchStyle();
    }, [firestore, styleId, isNew, toast, router]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isNew) {
                ...
                toast({ title: 'Created Blueprint', description: `"${name}" global blueprint has been initialized.` });
            } else {
    ...
    const commitRename = () => {
        ...
        if (trimmed && trimmed !== name) {
            setName(trimmed);
            toast({ title: 'Label updated (unsaved)', description: `Blueprint renamed to "${trimmed}". Remember to save.` });
        }
        setIsRenaming(false);
    };
    ...
    if (!style) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                ...
                <Button onClick={() => router.push('/admin/backoffice/messaging/styles')} className="rounded-xl">
                    Back to Blueprints Registry
                </Button>
            </div>
        );
    }
        ...
        <div className="space-y-6 text-left">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b pb-5">
                <div className="flex items-center gap-4 min-w-0">
                    <Button variant="ghost" size="icon" onClick={() => router.push('/admin/backoffice/messaging/styles')} className="rounded-xl shrink-0 text-muted-foreground hover:text-foreground">
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-2">
                            ...
                            <Badge variant="secondary" className="bg-primary/5 text-primary border-primary/20 font-bold uppercase tracking-widest text-[8px] h-5 rounded-lg shrink-0">
                                {isNew ? 'New' : 'System Blueprint'}
                            </Badge>
                        </div>
                        <p className="text-xs text-muted-foreground mt-0.5 truncate">
                            Control Plane — Configure platform-wide high-compatibility global wrapper templates.
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button size="sm" onClick={handleSave} disabled={saving || deleting || !name.trim()} className="rounded-xl h-9 text-xs shadow-lg shadow-primary/20 px-6">
                        {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1.5" /> : <Save className="h-3.5 w-3.5 mr-1.5" />} Save Blueprint
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                ...
                <div className="space-y-6">
                    <Card className="rounded-[1.5rem] border shadow-sm">
                        <CardContent className="p-6 space-y-4">
                            <div>
                                <h3 className="text-sm font-bold text-foreground">Wrapper Identity</h3>
                                <p className="text-xs text-muted-foreground mt-0.5">Determine the descriptive title for this global blueprint.</p>
                            </div>
                            <div className="space-y-2">
                                <Label className="text-xs font-semibold text-muted-foreground ml-1">Style Label</Label>
                                <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Standard Modern Blueprint" className="h-10 rounded-xl" />
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center px-1">
                            <Label className="text-xs font-bold text-muted-foreground flex items-center gap-1.5"><Code className="h-3.5 w-3.5 text-primary" /> HTML Source Wrapper</Label>
                        ...
                        <Textarea 
                            ...
                            placeholder="Write blueprint wrapper HTML..."
                    ...
                    <details className="group rounded-2xl border bg-card overflow-hidden" open>
                        <summary className="flex items-center justify-between cursor-pointer p-4 hover:bg-muted/30 transition-colors">
                            <span className="text-xs font-bold text-primary flex items-center gap-2"><Sparkles className="h-3.5 w-3.5" /> Blueprint Variables Reference</span>
                            <ChevronDown className="h-4 w-4 text-muted-foreground transition-transform group-open:rotate-180" />
                        </summary>
                        <div className="px-4 pb-4 pt-0 border-t">
                            <p className="text-[10px] font-semibold text-muted-foreground mb-3 mt-3">Click on any variable key to insert its placeholder tag inside the wrapper HTML blueprint.</p>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {[
                                    ...
                                    { key: 'brand_primary_color', desc: 'Primary accent hex' },
                ...
                <div className="space-y-2 lg:sticky lg:top-6">
                    <div className="flex items-center justify-between px-1">
                        <Label className="text-xs font-bold text-muted-foreground flex items-center gap-1.5"><Eye className="h-3.5 w-3.5 text-primary" /> Live Blueprint Rendering</Label>
                        <Badge variant="outline" className="text-[8px] font-bold h-5 uppercase rounded-lg">Fidelity Gateway</Badge>
                    </div>
                    ...
                    <AlertDialogHeader className="space-y-4">
                        ...
                        <div className="space-y-2 text-center">
                            ...
                            <AlertDialogDescription className="text-sm text-muted-foreground leading-relaxed">
                                This system blueprint wrapper is currently in active use by <span className="font-bold text-foreground">{styleInUseTemplates.length} message template{styleInUseTemplates.length !== 1 ? 's' : ''}</span>.
                                You cannot delete this global blueprint until those templates are reassigned to other style wrappers.
                            </AlertDialogDescription>
                        </div>
                    </AlertDialogHeader>
