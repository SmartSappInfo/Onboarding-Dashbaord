...
export function CreativeStudioNav() {
  ...
  return (
    <header className="w-full border-b border-slate-800 bg-slate-950/80 backdrop-blur-md sticky top-0 z-30 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        {/* Left Brand Badge */}
        <div className="flex items-center gap-3">
          <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-emerald-500 via-teal-500 to-cyan-500 flex items-center justify-center shadow-lg shadow-emerald-500/20 text-slate-950 font-black">
            <Wand2 className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-black tracking-tight text-white">Creative Studio</span>
        ...
        <nav className="hidden md:flex items-center gap-1 bg-slate-900/60 p-1 rounded-xl border border-slate-800/80">
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={cn('w-4 h-4', active ? 'text-slate-950' : 'text-slate-400')} />
                {item.label}
              </Link>
            );
          })}
        ...
        <div className="flex items-center gap-2">
          <Link href="/admin/backoffice/creative-studio">
            <Button
              variant="outline"
              size="sm"
              className="border-slate-800 bg-slate-900/50 text-slate-300 hover:text-white hover:bg-slate-800 font-bold text-xs h-9 min-h-[36px] rounded-xl active:scale-[0.97]"
            >
      ...
      <div className="md:hidden flex items-center justify-around border-t border-slate-850 px-2 py-1.5 bg-slate-950">
        {NAV_ITEMS.map((item) => {
          const active = isNavActive(item);
          const Icon = item.icon;
          return (
            <Link
              ...
              className={cn(
                ...
                active ? 'text-emerald-400 bg-emerald-500/10' : 'text-slate-400 hover:text-slate-200'
              )}
            >
