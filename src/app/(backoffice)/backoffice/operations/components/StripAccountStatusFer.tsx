...
export default function StripAccountStatusFer() {
  ...
  const handleExecute = () => {
    startTransition(async () => {
      ...
      try {
        ...
        const result = await executeStripAccountStatusFerAction(executorId);
        ...
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-emerald-500/15">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="font-semibold text-foreground text-sm">Strip accountStatus Protocol</h3>
            </div>
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              FER database-wide migration protocol to safely remove the deprecated{' '}
              <code className="bg-muted px-1 rounded text-[10px]">accountStatus</code> field from the{' '}
              <code className="bg-muted px-1 rounded text-[10px]">industryData</code> map on all SaaS entities.
            </p>
          </div>
          ...
          <div className="p-4 space-y-3">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2 rounded-lg bg-muted/60 border border-border px-3 py-2">
                ...
                <span className="text-xs font-mono text-foreground/80">entities.industryData</span>
                <Badge
                  variant="outline"
            ...
            {phase === 'confirm' && (
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/8 p-3 space-y-3">
                ...
                <p className="text-[10px] text-muted-foreground">
                  This scans all entities and uses Firestore FieldValue delete to clean up the deprecated field.
                </p>
                ...
                <div className="flex items-center gap-2 rounded-lg border border-emerald-500/20 bg-emerald-500/8 px-3 py-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      {lastResult.details?.entitiesStripped ?? 0} fields stripped
                    </p>
                  </div>
                </div>
                <Button
        ...
        {lastResult?.success && lastResult.details ? (
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <StatCard
                label="Scanned Entities"
                value={lastResult.details.entitiesScanned}
                sub="SaaS documents"
                accent="text-indigo-400"
              />
              <StatCard
                label="Stripped Fields"
                value={lastResult.details.entitiesStripped}
                sub="accountStatus keys"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-foreground font-medium">
                      {rec.success
                        ? `Stripped ${rec.details?.entitiesStripped ?? 0} docs`
                        : `Failed — ${rec.message}`}
                    </p>
                    <p className="text-[10px] text-muted-foreground">
                      Scanned: {rec.details?.entitiesScanned ?? 0} SaaS entities
                    </p>
                  </div>
