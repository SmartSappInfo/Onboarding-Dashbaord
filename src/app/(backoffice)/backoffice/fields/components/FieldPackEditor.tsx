...
export default function FieldPackEditor({ initialData }: { initialData?: PlatformFieldPack[] }) {
  ...
  const handleEdit = (pack: PlatformFieldPack) => {
    ...
    <div className="rounded-2xl border border-border bg-muted/30 p-6">
       <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Pre-configured bundles of custom fields mapped to specific industry verticals.</p>
          </div>
          {can('fields', 'create') && (
             <Button 
                size="sm" 
                onClick={handleCreate}
                className="bg-emerald-600 hover:bg-emerald-700 text-foreground rounded-xl h-9"
             >
       ...
       {isLoading ? (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1, 2, 3].map(i => (
                 <div key={i} className="h-48 bg-accent/40 rounded-xl animate-pulse border border-border" />
              ))}
           </div>
       ) : packs.length === 0 ? (
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {packs.map(pack => (
                 <div key={pack.id} className="p-5 rounded-xl border border-border bg-accent/30 relative group hover:border-slate-500 transition-all text-left">
                    ...
                    <div className="mt-4 space-y-2">
                       ...
                       <div className="flex flex-wrap gap-1.5">
                          {pack.industryCompatibility && pack.industryCompatibility.length > 0 ? (
                             pack.industryCompatibility.map(ind => (
                                <Badge key={ind} variant="outline" className="bg-emerald-500/5 border-emerald-500/10 text-emerald-400 text-[8px] uppercase px-1.5 py-0">
                                   {ind}
                                </Badge>
                             ))
                          ) : (
                             <span className="text-[9px] text-slate-600 italic">All Industries</span>
                          )}
                       </div>
                    </div>
                       ...
                       <div className="flex flex-col gap-1">
                          ...
                          <div className="flex items-center gap-1">
                            {pack.entityCompatibility?.map(ec => (
                               <Badge key={ec} variant="outline" className="bg-muted border-border text-foreground/80 text-[8px] uppercase px-1.5 py-0 h-auto">
                                  {ec === 'institution' ? 'ORG' : 'PSN'}
                               </Badge>
                            ))}
                          </div>
                       </div>
