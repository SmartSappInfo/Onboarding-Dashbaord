...
/**
 * Professional Unsubscribe & Preference Center Landing Page.
 * Phase 7: Compliance & Recipient Control.
 */
export default function UnsubscribePage({ params }: { params: Promise<{ id: string }> }) {
  const searchParams = useSearchParams();
  const { id: rawId } = React.use(params);
  const id = decodeURIComponent(rawId);
  const workspaceId = searchParams.get('ws') || 'global';
  const initialChannel = searchParams.get('c') || 'all';
            ...
            <Card className="border-none shadow-2xl overflow-hidden bg-white/80 backdrop-blur-sm">
              <CardHeader className="text-center pb-2">
                ...
                <CardDescription className="text-slate-600 mt-2">
                  We've successfully updated your communication preferences for <b>{workspaceId}</b>.
                </CardDescription>
              </CardHeader>
              ...
              <CardFooter className="bg-slate-50 border-t flex justify-center py-4">
                <p className="text-xs text-slate-400">© {new Date().getFullYear()} SmartSapp Cloud Services</p>
              </CardFooter>
            </Card>
          </motion.div>
        ) : (
            ...
            <Card className="border-none shadow-2xl">
              <CardHeader className="space-y-1">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider text-blue-600/80">Security & Privacy</span>
                </div>
                <CardTitle className="text-2xl font-bold tracking-tight">Email & SMS Preferences</CardTitle>
                <CardDescription>
                  Manage how we communicate with you. Unchecking a channel will stop all future non-critical messages.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid gap-6 py-6">
                  ...
                  <div className="flex items-start space-x-4 p-4 rounded-xl border bg-slate-50/50 hover:bg-white transition-colors cursor-pointer" onClick={() => setPreferences(prev => ({ ...prev, email: !prev.email }))}>
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      <p className="text-xs text-slate-500 leading-relaxed">
                        Weekly updates, campaign reports, and feature announcements.
                      </p>
                    </div>
                  </div>
                ...
                <div className="text-center">
                  <Button 
                    className="w-full bg-slate-900 hover:bg-slate-800 h-11 text-base font-medium rounded-xl transition-all active:scale-95" 
