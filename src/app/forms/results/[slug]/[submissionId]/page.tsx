...
async function getAuditData(slug: string, submissionId: string) {
    try {
        // 1. Resolve PDF Form Template (Check by ID first, then Slug)
        ...
        // 2. Resolve Specific Submission from the PDF's subcollection
        ...
        console.error(">>> [AUDIT:FETCH] Critical Error:", error);
        return null;
    }
}
...
export default async function SharedSubmissionDetailPage({ params }: { params: Promise<{ slug: string; submissionId: string }> }) {
    ...
    if (!slug || !submissionId || submissionId === 'undefined') {
        notFound();
    }
