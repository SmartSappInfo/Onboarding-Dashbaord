 ...
 * - Mobile & A11y first: min-h-[44px] touch targets, active:scale-[0.98], accessible focus states.
 * - Conforms to vercel-react-best-practices, emilkowal-animations, and frontend-design.
 */
...
export default function PortalContentCatalogClient({ slug }: PortalContentCatalogClientProps) {
  ...
  // 2. Server Action Fallback State
  ...
  // 3. Realtime Firestore Query for Content Items
  ...
  // 4. Memoized Filtered Content Items
  ...
  return (
    ...
    >
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href={`/portal/${slug}`}>
              <Button
                variant="ghost"
                size="icon"
                className="h-9 w-9 rounded-xl hover:bg-[var(--portal-surface)] text-[var(--portal-muted)] hover:text-foreground active:scale-[0.98] transition-transform"
            ...
            <Link href={`/portal/${slug}`} className="flex items-center gap-2.5">
              {branding.logoUrl ? (
              ...
              <span
                className="font-extrabold text-sm tracking-tight hidden sm:inline"
            ...
            <span className="text-xs font-bold text-[var(--portal-muted)] hidden sm:inline">Resource Vault</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="relative w-40 sm:w-56 text-left h-9 pl-8 pr-3 border border-[var(--portal-border)] bg-[var(--portal-surface)] text-xs text-[var(--portal-muted)] hover:text-foreground flex items-center justify-between transition-colors shadow-2xs focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-primary"
              style={{ borderRadius: radiusCss }}
            >
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5" />
              <span className="truncate">Search catalog...</span>
              <kbd className="text-[9px] bg-muted/60 px-1 py-0.5 rounded border border-border hidden sm:inline">⌘K</kbd>
            </button>
            ...
            {user ? (
              <Link href={`/portal/${slug}/dashboard`}>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-9 rounded-xl font-bold text-xs gap-1.5 active:scale-[0.98] transition-transform"
                >
                  <LayoutDashboard className="w-3.5 h-3.5 text-[var(--portal-primary)]" />
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-6 md:p-10 space-y-8">
        {/* Banner Section */}
        <div
          className="p-8 sm:p-10 rounded-3xl border-2 border-[var(--portal-border)] bg-[var(--portal-surface)] space-y-3 relative overflow-hidden"
          style={{ borderRadius: radiusCss }}
        >
          <Badge
            variant="outline"
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 border-b border-[var(--portal-border)] pb-4">
            ...
            {TYPE_FILTER_TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  className={`min-h-[44px] px-3.5 rounded-xl text-xs font-bold flex items-center gap-2 whitespace-nowrap transition-all duration-150 active:scale-[0.98] ${
                    isActive
                      ? 'text-white shadow-xs'
                      : 'text-[var(--portal-muted)] hover:text-foreground hover:bg-[var(--portal-surface)]'
                  }`}
                  style={{
          ...
          <div className="relative w-full md:w-72">
            ...
            <Input
              ...
              className="pl-9 pr-8 h-10 rounded-xl text-xs bg-[var(--portal-surface)] border-[var(--portal-border)] font-medium"
              style={{ borderRadius: radiusCss }}
            />
            {searchQuery && (
              <button
                ...
                className="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 text-[var(--portal-muted)] hover:text-foreground"
        ...
        {isLoadingContent && effectiveItems.length === 0 ? (
          ...
          >
            ...
            <h3 className="font-bold text-base text-foreground">No resources match your selection</h3>
            ...
            {(selectedType !== 'all' || searchQuery) && (
              <Button
                ...
                className="min-h-[44px] rounded-xl font-bold text-xs mt-2 active:scale-[0.98] transition-transform"
              >
                Reset Filters
              </Button>
            )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.map(item => {
              ...
              const isGated =
                item.visibility === 'membership_required' ||
                item.visibility === 'invite_only' ||
                Boolean(item.accessRoles && item.accessRoles.length > 0);
              const targetUrl = `/portal/${slug}/content/${item.type}/${item.slug}`;

              return (
                <Card
                  key={item.id}
                  className="border-2 border-[var(--portal-border)] bg-[var(--portal-surface)] p-6 space-y-4 hover:shadow-xl hover:border-[var(--portal-primary)] transition-all duration-200 active:scale-[0.98] flex flex-col justify-between group"
                  style={{ borderRadius: radiusCss }}
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        {isGated && (
                          <Badge variant="outline" className="text-[10px] font-bold text-amber-600 border-amber-500/30 gap-1 bg-amber-500/5">
                            <Lock className="w-3 h-3" /> Member
                          </Badge>
                        )}
                        <Badge variant="secondary" className="text-[10px] font-bold uppercase tracking-wider capitalize">
                          {item.type}
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1">
                      <Link href={targetUrl}>
                        <h3 className="font-bold text-base text-foreground group-hover:text-[var(--portal-primary)] transition-colors line-clamp-2 leading-snug">
                          {item.title}
                        </h3>
                      </Link>
                      {item.summary && (
                    ...
                    {item.tags && item.tags.length > 0 && (
                      <div className="flex flex-wrap items-center gap-1 pt-1">
                        {item.tags.slice(0, 3).map((tag, idx) => (
                          <span
                            key={idx}
                            className="text-[10px] font-medium text-[var(--portal-muted)] bg-background/60 px-2 py-0.5 rounded-md border border-[var(--portal-border)]"
                          >
                            #{tag}
                          </span>
                        ))}
                  ...
                  <div className="pt-3 border-t border-[var(--portal-border)] flex items-center justify-between gap-2">
                    {item.media?.downloadUrl ? (
                      <a
                        ...
                        className="min-h-[44px] flex items-center gap-1.5 text-xs font-bold text-[var(--portal-primary)] hover:opacity-80 active:scale-[0.98] transition-transform"
                      >
                    ...
                    <Link href={targetUrl}>
                      <Button
                        size="sm"
                        className="min-h-[44px] px-3.5 rounded-xl font-bold text-xs gap-1.5 text-white active:scale-[0.98] transition-transform"
      ...
      <footer className="border-t border-[var(--portal-border)] bg-[var(--portal-surface)] px-6 py-8 transition-colors mt-12">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[var(--portal-muted)]">
          ...
          <div className="flex items-center gap-4">
            <Link href={`/portal/${slug}`} className="hover:text-foreground transition-colors">
              Portal Home
            </Link>
            <Link href={`/portal/${slug}/learn`} className="hover:text-foreground transition-colors">
              Curriculum
            </Link>
            <Link href={`/portal/${slug}/community`} className="hover:text-foreground transition-colors">
              Community
            </Link>
          </div>
        </div>
