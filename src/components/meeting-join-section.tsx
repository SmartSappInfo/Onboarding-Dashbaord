...
function MeetingJoinSectionInner({ meeting, schoolId }: { meeting: Meeting; schoolId: string }) {
  ...
  if (registrationRequired) {
    ...
    return <MeetingRegistrationForm meeting={meeting} schoolId={schoolId} onRegistered={setToken} />;
  }
...
export default function MeetingJoinSection({ meeting, schoolId }: { meeting: Meeting; schoolId: string }) {
  return (
    <Suspense fallback={
      <div className="w-full max-w-md mx-auto md:mx-0 p-10 bg-white/5 backdrop-blur-md rounded-[2.5rem] border border-white/10 text-center shadow-2xl flex items-center justify-center">
        <Loader2 className="h-8 w-8 animate-spin text-primary" />
      ...
      <MeetingJoinSectionInner meeting={meeting} schoolId={schoolId} />
    </Suspense>
  );
}
