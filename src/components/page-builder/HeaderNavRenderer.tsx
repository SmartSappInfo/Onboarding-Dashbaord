 ...
 * (Left, Center, Right), and rich nested dropdown popovers supporting subtitles, Lucide icons, and badges.
 *
 * CAUTION:
 * Ensure pointer events and popover positioning avoid clipping inside header overflow containers.
 *
 * TESTABILITY:
 * Test style swapping, alignment updates, and hover/click interactions on dropdown sub-items.
 */
...
import { 
  ...
  Sparkles, Folder, Code, User, Phone, FileText, HelpCircle, Layers, Link as LinkIcon
} from 'lucide-react';
        ...
        return (
          ...
          >
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 transition-all duration-200 cursor-pointer min-h-[40px] px-3 py-1.5 rounded-lg select-none",
                ...
                // Style Preset 2: Underline Slide
                navStyle === 'underline_slide' && "text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white relative after:content-[''] after:absolute after:bottom-0 after:left-3 after:right-3 after:h-[2px] after:bg-primary after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-200",
                
                // Style Preset 3: Pill Tabs
                navStyle === 'pill_tabs' && "text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-zinc-800/80 rounded-full px-3.5 py-1.5",
                
                // Style Preset 4: Glass Cards
                navStyle === 'glass_cards' && "text-slate-700 dark:text-slate-200 bg-slate-900/5 dark:bg-white/5 border border-slate-200/50 dark:border-white/10 hover:border-primary/50 shadow-sm backdrop-blur-md rounded-xl px-3.5 py-1.5",
                
                // Style Preset 5: Mega Menu
                navStyle === 'mega_menu' && "text-slate-800 dark:text-slate-100 font-bold hover:text-primary transition-colors px-3 py-1.5 rounded-lg hover:bg-slate-100/60 dark:hover:bg-zinc-800/50",

                isOpen && "text-primary dark:text-primary"
              )}
            >
              {renderNavItemIcon(item.icon, "h-3.5 w-3.5 text-primary shrink-0")}
            ...
            {hasChildren && (
              <div
                className={cn(
                  "absolute top-full left-0 mt-2 z-50 transition-all duration-200 transform origin-top-left",
                  isOpen
              ...
              >
                <div
                  className={cn(
                    "p-2 rounded-2xl shadow-2xl border backdrop-blur-xl transition-all",
                      ...
                      <div className="grid grid-cols-1 gap-1 pt-1">
                        {children.map((child) => (
                          <button
                            ...
                            className="flex items-start gap-3 p-2.5 rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-900/90 transition-all text-left group/child cursor-pointer"
                          >
                            <div
                              className="h-8 w-8 rounded-lg flex items-center justify-center shrink-0 bg-slate-100 dark:bg-zinc-850 border border-slate-200/80 dark:border-zinc-800 group-hover/child:bg-primary group-hover/child:text-white transition-colors"
                    ...
                    <div className="space-y-1 p-1">
                      {children.map((child) => (
                        <button
                          ...
                          className="w-full flex items-center justify-between p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-zinc-900 transition-colors text-left group/child cursor-pointer"
                        >
                          ...
                          {child.badge ? (
                            ...
                            <ExternalLink className="h-3 w-3 text-slate-400 opacity-0 group-hover/child:opacity-100 transition-opacity shrink-0" />
                          )}
                        </button>
                      ))}
                    </div>
