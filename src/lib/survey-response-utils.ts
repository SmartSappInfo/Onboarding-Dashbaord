...
/**
 * Leading prefix added by the survey uploader, stripped so respondents and reviewers see
 * the file name they recognise.
 *
 * Two shapes are accepted:
 *   legacy  `1740001234567-Staff_Roster.xlsx`
 *   current `1740001234567-a1b2c3-Staff_Roster.xlsx`
 *
 * The random segment was added because the timestamp alone is not unique — two
 * respondents uploading the same file name in the same millisecond overwrote each other.
 *
 * CAUTION: the collision segment is matched as exactly 6 lower-case alphanumerics, so a
 * legacy file genuinely named `<timestamp>-abcdef-report.pdf` would lose `abcdef` from
 * its display name. The stored file is untouched; only the label is affected.
 */
const UPLOAD_PREFIX_PATTERN = /^\d{10,14}(?:-[a-z0-9]{6})?-(.+)$/;
...
export function extractFileNameFromStorageUrl(urlStr: string): string {
  try {
    ...
    const prefixMatch = rawName.match(UPLOAD_PREFIX_PATTERN);
    if (prefixMatch) {
