...
export async function sendReceiptAcknowledgementAction(payload: {
    ...
    const { name, school, phone, email, amount, documentUrl } = payload;
    ...
    try {
                            ...
                            <table class="content" width="100%" border="0" cellspacing="0" cellpadding="0">
                                ...
                                <tr>
                                    <td class="text">
                                        ...
                                        ${documentUrl ? `<strong>Attached Document Proof:</strong> <a href="${documentUrl}" target="_blank" style="color: #3A86FF; font-weight: 700;">View Uploaded Document</a><br><br>` : ''}
        ...
        // 2. Send Finance Team Notification Email
        const financeEmailHtml = `
            ...
            Email: <strong class="highlight">${email}</strong><br>
            ${documentUrl ? `Attached Document / Proof: <a href="${documentUrl}" target="_blank" style="color: #3A86FF; font-weight: bold;">View Uploaded Document</a><br>` : ''}<br>
        ...
        await sendEmail({
            to: ['info@smartsapp.com', 'accounts@minex360.com'],
        ...
        // 4. Send SmartSapp Team & Org Admin SMS Notification
        const docText = documentUrl ? `\nDocument: ${documentUrl}` : '';
