...
interface ExecutionContext {
    schoolId?: string;
    entityId?: string; // New field for entity-based triggers
...
async function resumeAutomationRun(job: AutomationJob) {
    try {
        ...
        const context: ExecutionContext = {
            ...
            entityId: job.payload.entityId, // Support entityId in resumed jobs
...
async function executeAutomation(automation: Automation, triggerPayload: Record<string, any>) {
    ...
    const context: ExecutionContext = {
        ...
        entityId: triggerPayload.entityId, // Support entityId in triggers
...
async function handleSendMessage(config: any, context: ExecutionContext) {
    ...
    // Resolve contact using entityId or schoolId (Requirement 14.3)
    const contactId = context.entityId || context.schoolId;
...
async function handleCreateTask(config: any, context: ExecutionContext) {
    ...
    // Use adapter to resolve contact information (Requirement 18)
    // Prefer entityId over schoolId (Requirement 14.2)
    let schoolId: string | null | undefined = context.schoolId || null;
    let schoolName: string | null | undefined = context.payload.schoolName || null;
    ...
    if (entityId) {
        ...
        if (contact) {
            ...
            // Populate schoolId for backward compatibility if available
            if (contact.schoolData?.id) {
                schoolId = contact.schoolData.id;
            }
        }
    } else if (schoolId) {
        // Fallback: resolve from schoolId
        const contact = await resolveContact(schoolId, context.workspaceId);
        if (contact) {
            schoolName = contact.name;
    ...
    // Populate both schoolId and entityId (dual-write pattern, Requirement 14.2)
    await adminDb.collection('tasks').add({
        ...
        schoolName, // Dual-write: legacy field
...
async function handleUpdateSchool(config: any, context: ExecutionContext) {
    // Prefer entityId over schoolId (Requirement 14.4)
    const contactId = context.entityId || context.schoolId;
    if (!contactId) throw new Error("Cannot update contact: Context missing both entityId and schoolId.");
