...
export class FieldsVariablesService {
  ...
  static async getVariableValuesMap(context: DataResolutionContext): Promise<Map<string, unknown>> {
    ...
    if (context.surveyId && surveySnap?.exists) {
      try {
        ...
        if (responseId && responseData) {
          ...
          const rName = responseData.respondentName || leadDetails?.name || leadDetails?.contactName || valuesMap.get('contact_name') || valuesMap.get('entity_name');
          if (rName) {
            valuesMap.set('respondent_name', String(rName));
            valuesMap.set('respondentName', String(rName));
          }
          ...
          // Dynamically resolve matched outcome rule if result_message is not explicit on response document
          let resultMsg = String(responseData.resultMessage || responseData.message || '');
          let resultTitle = String(responseData.resultTitle || responseData.outcome || '');
          let resultDesc = String(responseData.resultDescription || '');
          let outcomeLabel = String(responseData.outcome || responseData.outcomeLabel || '');
          ...
          if (!resultMsg && survey.scoringEnabled && survey.resultRules?.length && responseData.score !== undefined) {
            ...
            if (matched) {
              ...
              resultDesc = matched.description || '';
              outcomeLabel = matched.label || matched.title || '';
            }
          }

          valuesMap.set('result_message', resultMsg);
          valuesMap.set('resultMessage', resultMsg);
          valuesMap.set('result_title', resultTitle);
          valuesMap.set('resultTitle', resultTitle);
          valuesMap.set('result_description', resultDesc);
          valuesMap.set('resultDescription', resultDesc);
          valuesMap.set('outcome_label', outcomeLabel);
          valuesMap.set('outcomeLabel', outcomeLabel);
          valuesMap.set('survey_result', resultTitle || resultMsg);
    ...
    if (!valuesMap.get('respondent_name') || valuesMap.get('respondent_name') === '') {
      const respFallback = valuesMap.get('respondentName') || valuesMap.get('contact_name') || valuesMap.get('entity_name') || valuesMap.get('PRIMARY_CONTACT_NAME') || valuesMap.get('CURRENT_CONTACT_NAME') || 'Respondent';
      valuesMap.set('respondent_name', respFallback);
      valuesMap.set('respondentName', respFallback);
    }

    if (!valuesMap.get('result_message') || valuesMap.get('result_message') === '') {
      const resFallback = valuesMap.get('resultMessage') || valuesMap.get('result_title') || valuesMap.get('resultTitle') || valuesMap.get('outcome_label') || valuesMap.get('outcomeLabel') || valuesMap.get('survey_result') || '';
      valuesMap.set('result_message', resFallback);
      valuesMap.set('resultMessage', resFallback);
    }
