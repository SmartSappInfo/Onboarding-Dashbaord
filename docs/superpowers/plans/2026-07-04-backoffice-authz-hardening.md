...
- [ ] **Step 0.1 — Error util.** Create `src/lib/backoffice/backoffice-errors.ts`:
...
- [ ] **Step 0.2 — Write failing test.** Create `src/lib/backoffice/__tests__/backoffice-auth.test.ts` (mock `firebase-admin` `adminAuth.verifyIdToken` + `adminDb`, mirror the mocking style in `industry-propagation.test.ts`):
...
- [ ] **Step 0.3 — Run test, verify FAIL.** `pnpm vitest run src/lib/backoffice/__tests__/backoffice-auth.test.ts` → FAIL (module not found).

- [ ] **Step 0.4 — Implement helper.** Create `src/lib/backoffice/backoffice-auth.ts`:
...
- [ ] **Step 0.5 — Run test, verify PASS.** `pnpm vitest run src/lib/backoffice/__tests__/backoffice-auth.test.ts` → PASS.

- [ ] **Step 0.6 — Client token hook.** Create `src/hooks/use-backoffice-token.ts`:
...
- [ ] **Step 0.7 — Matrix parity test** (R7). Add to `backoffice-auth.test.ts`: import server `ROLE_MATRIX` (export it from `backoffice-rbac.ts`) and the client mirror; assert deep structural equality of role→module→action sets.
...
- [ ] **Step 1.1 — Authz test** for one action per file in `__tests__/backoffice-actions-authz.test.ts` (feature kill switch, ai keys save): allowed role passes, wrong role → `forbidden`. Run → FAIL.
- [ ] **Step 1.2 — feature-actions:** convert `listAllFeatures`, `getFeatureDetail` (`features:view`), `updateFeatureRolloutRules` (`features:edit`), `toggleFeatureKillSwitch` (`features:execute`). Replace all `catch (error: any)` → `unknown` + `getErrorMessage`.
- [ ] **Step 1.3 — job-actions:** replace `resolveActorFromToken` internals with a call to `resolveBackofficeActor`; add `authorizeBackoffice(idToken,'operations','execute')` to `createJob`/`cancelJob`/`executeJob` (retry entrypoint) and `operations:view` to `listAllJobs`. Preserve `after(() => executeJob(id, actor))` capturing the resolved actor (R9). Remove the local `resolveActorFromToken`/`BackofficeRole` single-role logic.
- [ ] **Step 1.4 — ai-actions:** `getGlobalAiKeys`/`getGlobalAiConfig` → `settings:view`; `saveGlobalAiKeys`/`saveGlobalAiConfig` → `settings:edit`. Keep the zod `.parse` **before** auth (validate → auth → mutate). Keep `••••••••` sentinel logic untouched (encryption is Phase 5).
- [ ] **Step 1.5 — Call sites:** import `useBackofficeToken`; replace hand-built `actor` objects with `const idToken = await getToken()`; wrap calls in try/catch → toast on `Forbidden`, ignore `NotAuthenticatedError` on mount. Use `useTransition` for pending state (emilkowal — no <200ms spinner flash).
- [ ] **Step 1.6 — Run authz tests → PASS.**
- [ ] **Step 1.7 — Verify:** `pnpm typecheck && pnpm lint && pnpm vitest run src/lib/backoffice`. Fix all errors (stale callers will fail typecheck — migrate them). 
- [ ] **Step 1.8 — Manual smoke:** as super_admin toggle a feature + save AI config (works); temporarily set a test user to `readonly_auditor` and confirm `Forbidden`.
- [ ] **Step 1.9 — Commit.**
```bash
git add -A
...
- [ ] **Step 2.0 — Resolve Open Question Q1** (suspend / clear-logs severity) before coding.
- [ ] **Step 2.1 — Authz test** for `suspendOrganization` + `saveProviderSetting`. Run → FAIL.
- [ ] **Step 2.2 — org-actions:** convert per matrix — reads `organizations:view`; `update`/`toggleOrganizationActivityLogging` → `edit`; `create*`/`shareOrgSetupInviteAction` → `create`; `suspend`/`restore`/`clearOrganizationActivityLogs` → `execute` (per Q1). Preserve join-token TTL/single-use logic and confirm dialogs. Replace `catch (error: any)`.
- [ ] **Step 2.3 — provider-actions:** `listProviderSettings` → `settings:view`; `saveProviderSetting` → `settings:edit`. Keep the provider+type upsert query (needs the Phase 6 index).
- [ ] **Step 2.4 — Call sites** migrate to `getToken()`; keep destructive-action confirmations.
- [ ] **Step 2.5 — Verify + smoke + Commit.**
```bash
...
- [ ] **Step 3.1 — Authz test** for `savePlatformIndustryFieldGroup` + `deletePlatformIndustryFieldGroup`. Run → FAIL.
- [ ] **Step 3.2 — field-actions:** reads `fields:view`; saves `fields:edit`; `deletePlatformIndustryFieldGroup` `fields:delete`. **Auth must pass before `propagateIndustryGroupChanges` runs** — put `authorizeBackoffice` at the top of the try.
- [ ] **Step 3.3 — template/asset/workspace actions** per matrix.
- [ ] **Step 3.4 — Call sites** incl. the external `RolesClient.tsx` (grep for the exact imported action; migrate its call).
- [ ] **Step 3.5 — Verify + smoke** (edit a field pack, confirm workspace propagation still fires) **+ Commit.**
```bash
...
- [ ] **Step 4.1 — Authz test** for `fetchAuditLogs` (`audit:view`). Run → FAIL.
- [ ] **Step 4.2 — audit-actions:** `fetchAuditLogs`/`queryAuditLogs` take `idToken` first arg, `audit:view`. Keep `queryAuditLogs` usable by internal callers by exposing a token-less `queryAuditLogsInternal` used only by other **already-authorized** server actions (avoid double-auth); the public export requires token.
- [ ] **Step 4.3 — Sweep:** confirm every exported `list*/get*/fetch*` across all 9 files now begins with an `authorizeBackoffice(idToken, module, 'view')`. (Grep checklist in Step 4.5.)
- [ ] **Step 4.4 — Call sites** pass `getToken()`; audit viewer handles empty/forbidden gracefully.
- [ ] **Step 4.5 — Verify** including grep gate:
```bash
...
- [ ] **Step 5.1 — Preflight:** confirm `isVaultConfigured()` true in every env; `saveGlobalAiKeys` throws (not silent plaintext) when false.
- [ ] **Step 5.2 — Failing test:** save a key → stored value is an envelope (has `version`/`iv`/`ciphertext`), not plaintext; read path decrypts to original; `••••••••` sentinel leaves value unchanged.
- [ ] **Step 5.3 — Implement:** wrap secret fields with `crypto-vault` `encrypt()` on write, `decrypt()` on read. Sentinel logic compares against **decrypted** current value. Never log plaintext (keep masking in audit).
- [ ] **Step 5.4 — Migration:** add `encrypt_platform_secrets` to `PlatformJobType`; implement dry-run-capable job that encrypts existing plaintext rows (skip already-versioned envelopes). Run dry-run, review, then real run.
...
- [ ] **Step 6.1** — Add the 8 index entries from *Required Firebase Artifacts §1*.
- [ ] **Step 6.2** — Add the `platform_*` deny + audit-immutability rules from §2.
- [ ] **Step 6.3 — Rules tests** (if `@firebase/rules-unit-testing` present or via `firebase emulators:exec`): assert a signed-in non-admin client SDK read of `platform_features`/`platform_audit_logs` is denied; `platform_audit_logs` write denied.
- [ ] **Step 6.4 — Deploy:**
```bash
firebase deploy --only firestore:indexes,firestore:rules
```
- [ ] **Step 6.5 — Verify** audit filtering in the UI no longer throws `FAILED_PRECONDITION`.
- [ ] **Step 6.6 — Commit.**
```bash
...
1. **Q1 (Phase 2):** Should `suspendOrganization` / `restoreOrganization` / `clearOrganizationActivityLogs` be **super-admin-only** (`organizations:execute`) or available to `support_admin`/`tenant_admin_ops` (`organizations:edit`)? Plan assumes execute (super-admin-only).
2. **Q2 (Phase 5):** Confirm the envelope vault env key is provisioned in prod/staging/dev, or should Phase 5 be deferred until it is?
3. **Q3 (Phase 7):** Is `src/app/admin/backoffice/messaging/*` the canonical messaging surface or legacy to delete?
