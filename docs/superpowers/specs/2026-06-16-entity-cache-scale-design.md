...
- **6.0 Foundation (no consumer change). ✅ BUILT (verify locally).**
  - [x] Pure domain [contact-projection-domain.ts](src/lib/contacts/contact-projection-domain.ts) — `flattenEntityContacts`/`contactDocId`/`diffContactDocs`/`reachField`/`channelsOf` + `ContactDoc`/`AudienceSegment` types. **12 unit tests pass.**
  - [x] **Channel reachability stored as booleans** (`hasEmail`/`hasPhone`), not a `channels` array — so `workspaceTags` is the *only* array field and channel+tag queries stay indexable (Firestore = 1 array field/index).
  - [x] `workspace_contacts` security rule (server-only writes; reads mirror `workspace_entities`) in [firestore.rules](firestore.rules).
  - [x] 6 composite indexes (channel × {name/email order} × optional tag) in [firestore.indexes.json](firestore.indexes.json).
  - [x] Resumable [backfill-workspace-contacts.ts](src/lib/contacts/backfill-workspace-contacts.ts) (batch-flush before the 500-write limit) + blue **Backfill Contacts** card on [/admin/seeds](src/app/admin/seeds/SeedsClient.tsx).
  - [ ] **▶ RUN LOCALLY:** `npm run typecheck` · `firebase deploy --only firestore:indexes,firestore:rules` · click **Backfill Contacts** on /admin/seeds.
  - [ ] **Commit:** `feat(contacts): workspace_contacts projection foundation + backfill`.
- **6.1 Write path. ✅ BUILT (verify locally).**
  - [x] [contact-projection-writer.ts](src/lib/contacts/contact-projection-writer.ts) — `syncContactProjectionForWE` (read existing rows → `diffContactDocs` → batch upsert/delete; **never throws into the caller**), `syncContactProjectionForEntityWorkspace` (tag paths), `deleteContactProjectionForEntity` (cascade), `reconcileWorkspaceContacts` (cursor-resumable heal).
  - [x] **Design choice:** rather than rewrite ~20 contact writers, the projection hooks the *convergence point* — every contact mutation already lands as `workspace_entities.entityContacts`. Hooked: `entity-actions` create + update; WE delete (both sites in `workspace-entity-actions`); `bulk-upload-actions`; `scoped-tag-actions` apply/remove (also covers `lead-actions`, which tags via `applyTagAction`). `import-service` creates with empty contacts (no-op; contacts arrive later via the hooked `updateEntity`).
  - [x] **Property test** ([contact-projection.property.test.ts](src/lib/contacts/__tests__/contact-projection.property.test.ts), fast-check 200 runs): from ANY prior projection state, one sync makes rows == `flatten(WE)` — the formal drift-convergence guarantee. **14/14 contacts tests pass.**
  - [x] **Reconcile** + **Backfill** buttons on [/admin/seeds](src/app/admin/seeds/SeedsClient.tsx) (backfill = fast upsert-only seed; reconcile = upsert + delete heal).
  - [ ] **Deferred (reconcile-covered, infrequent):** bulk `tag-actions.ts` paths — tag *definition* delete fan-out + tag merge. Will hook the exact path AssignContactsToTagDialog uses during 6.3; until then the reconcile job heals tag drift. Document only.
  - [ ] **▶ RUN LOCALLY:** `npm run typecheck` · `npx vitest run src/lib/contacts`.
  - [ ] **Commit:** `feat(contacts): single-writer projection sync + reconcile`.
- **6.2 Query layer. ✅ BUILT (verify locally).**
  - [x] **Boolean reachability model** (linter/design refinement): `ContactDoc` stores `hasEmail`/`hasPhone` flags, **not** a `channels[]` array — Firestore allows only one array field per composite index, reserved for `workspaceTags` (array-contains). `reachField(channel)` maps a segment channel to its flag; `channelsOf(doc)` is the display inverse.
  - [x] **Pure query plan** `contactSegmentToQuerySpec(workspaceId, segment)` ([contact-projection-domain.ts](src/lib/contacts/contact-projection-domain.ts)) — single tested source of filter/order/range decisions, pinned to the deployed indexes (a tag filter forces `nameLower` search; email-looking search ranges on `emailLower`). 6 spec tests.
  - [x] **Client hook** [useContactSearch](src/hooks/use-contact-search.ts) — paged `workspace_contacts` query via rules (mirrors `useEntitySearch`); opt-in `totalCount` via `getCountFromServer` = what "Select All Match" selects, no ids in the browser.
  - [x] **Server repo** [contact-repository.ts](src/lib/contacts/contact-repository.ts) — `countSegment`, `pageSegmentRecipients` (stable order-field + doc-id cursor), `resolveSegmentRecipients` (internally paged, capped) for the send pipeline. Same spec builder as the client.
  - [x] Composite indexes already in [firestore.indexes.json](firestore.indexes.json) (6.0); `contact-adapter.ts` scans left intact until their callers migrate (6.7).
  - [ ] **▶ RUN LOCALLY:** `npm run typecheck` · `npx vitest run src/lib/contacts` (20 tests). **Deploy indexes** before runtime use: `firebase deploy --only firestore:indexes,firestore:rules`, then run **Backfill Contacts** on `/admin/seeds`.
  - [ ] **Commit:** `feat(contacts): server contact search/segment repository + useContactSearch`.
- **6.3 Light consumers. ✅ BUILT (verify locally).**
  - [x] **ContactScopeSelector** — needed only the *distinct entity types* in the workspace, which equals the workspace's `contactScope` (enforced at entity-create). Reads `activeWorkspace.contactScope` instead of slicing 100 entities — no projection query needed, and more correct (the slice could miss a type).
  - [x] **deals/[id]/page contact-add** — `useEntitySearch` gated on `isAddContactOpen`; results filtered to exclude already-linked entities; "Load more" added. (Entity-level picker → name-prefix search, per O3.)
  - [x] **AssignContactsToTagDialog** — `useEntitySearch` (gated on `open`); selection now keeps the full entity objects (keyed by doc id) so preview badges survive across pages; "Load more" added.
  - [x] **Tag write hooked:** `bulkApplyTagsAction` (the dialog's write) now re-projects the re-tagged `workspace_entities` — closing the deferred 6.1 tag path for this consumer. (Single `applyTagsAction`/`removeTagsAction` + tag-*definition* delete fan-out remain reconcile-covered.)
  - [ ] **▶ RUN LOCALLY:** `npm run typecheck`. Needs indexes deployed + backfill run to function at runtime.
  - [ ] **Commit per consumer** (or one: `feat(contacts): migrate light audience consumers to projection`).
- **6.4 Recipient builders. ✅ DONE (verify locally).**
  - [x] **ManualContactSelector** (the archetype) — paged `useContactSearch`; **"Select All Match" resolves server-side** via `resolveSegmentRecipients` (capped + truncation toast). Kept the `onChange(array)` contract, so the send pipeline is untouched; the parent `AudienceSelector` (`channel`/`selectedContacts` shape) is drop-in compatible. Numbered pagination → load-more.
  - [x] **Bug fixed (caught by the property test):** duplicate `entityContacts[].id` within one entity collapsed to the same projection doc id; `flattenEntityContacts` now dedupes by doc id (last-wins, matching `batch.set`). 20/20 contacts tests pass.
  - [x] **campaign-wizard (messaging)** — the `useEntityCache` import was **dead** (never called); removed. Call-centre `CampaignWizardClient` uses the (migrated) `ManualContactSelector`, no direct cache read.
  - [x] **ComposerWizard → EntitySelector** — EntitySelector now uses `useEntitySearch` (paged list, load-more) + `useEntityResolver` (selected strip) + `getEffectiveContactTypes` (contact-type chips); dropped the `entities`/`isLoading` props. ComposerWizard removed the `weRaw`+`entitiesRaw` join, `workspaceEntities`, `isCombinedLoading`, and the send-loop `entityMap` (display name now via the existing `resolveContact`). "Select all" → "Select all loaded" (bounded; broadcast-scale uses the tag-audience path). **`EntitySelector.test.tsx` rewritten** to mock the hooks — 16/16 pass.
  - [x] **meetings/[id]/invitations** (the hardest) — added `isSignatory` to the projection (+ flatten/backfill/reconcile pick it up). New server resolver `resolveInvitationRecipients` narrows by the indexable parts (workspaceId + status, + first tag under AND logic) and **post-filters the rest in memory, paginated** (multi-tag AND/OR, exclude, scope/roles, multi-channel, assignee). The predicate + primary-per-entity grouping are **pure + unit-tested** (`matchesInvitationFilter`, `pickPrimaryPerEntity`). `filteredRecipients` is now debounced server-resolved state; `availableRoles` → `getEffectiveContactTypes`. Dropped both the WE-cache read and the org-wide `entities` collection load.
  - [x] **`isSignatory` schema bump** → re-run **Reconcile** on `/admin/seeds` so existing projection rows gain the field (signatories scope is empty until reconciled).
  - [ ] **▶ RUN LOCALLY** typecheck + a send smoke test each. **Commit per consumer.**
  - **Note:** fixed a *pre-existing* parse error in `CallCentreClient.tsx` (3 `wrapHref('…${id}` template literals opened with a quote, closed with a backtick) — unrelated to this epic, but it was blocking the whole-project typecheck.
- **6.5 Reports. ✅ DONE.** ContractsClient + ReportsClient migrated. ReportsClient → server action [getReportAggregates](src/lib/reports/report-actions.ts) (field-projected via the dashboard repo; velocity buckets + per-zone count/capacity computed server-side, only the small result crosses the wire). ContractsClient → `useEntitySearch` (paged + load-more), dropped the WE-cache read **and** the org-wide `entities` load (zone/signatory now from denormalized WE fields); coverage stats are global (total via `count()`, signed/pending from the contracts collection); status filter + select-all-unprepared act on loaded rows.
- **6.6 Retire subscription. ✅ DONE.** Grep confirms zero `useEntityCache`/`useSortedEntities` readers. [EntityCacheContext.tsx](src/context/EntityCacheContext.tsx) rewritten to **resolver-only**: removed the unbounded `onSnapshot`, the LRU full-set module cache, the `requestFullSet` gate, and the `useEntityCache`/`useSortedEntities`/`useActiveEntities`/`useEntityLookup` hooks + test utilities. Kept `resolve` / `useEntityResolver` / `useEntityByDocId`. **The platform-wide freeze is gone.** ▶ RUN LOCALLY: `npm run build`.
- **6.7 Cleanup. ✅ DONE.** The full-scan `contact-adapter` functions `getWorkspaceContacts` / `searchContacts` / `contactExists` (zero production callers) are **deleted**, along with their `describe` blocks + imports in `contact-adapter.test.ts` and `contact-adapter-new-methods.test.ts`. `resolveContact` (single-entity, not a scan; ~25 callers) stays. **Adapter + contacts suites green: 31/31.** No full-scan contact reader remains anywhere in the codebase.
